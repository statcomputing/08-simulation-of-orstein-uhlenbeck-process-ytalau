...
(poimean <- integrate(intfun, lower = 0, upper = 5))
```

$N(5)$ is a Poisson random variable with mean `r poimean$value`.

### simulate from the Poisson process
...
plotfun <- function(x, tmax = 5) {
    poimean <- integrate(intfun, lower = 0, upper = tmax)
    intfun(x)/poimean$value
}
library(ggplot2)
simdat <- data.frame(x = unlist(replicate(10000, rnhpp(intfun, intfun(5), 5))))
ggplot(data = simdat, aes(x)) + 
    geom_density(colour = "blue") +
    geom_function(fun = plotfun, colour = "red") +
     xlim(0, 5)
```
